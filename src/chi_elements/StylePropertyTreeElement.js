...
} from './StylesSidebarPane.js'; // eslint-disable-line no-unused-vars
...
export class StylePropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  async _addColorContrastInfo(swatch) {
    ...
    const contrastInfo = new ColorPicker.ContrastInfo.ContrastInfo(
      await cssModel.backgroundColorsPromise(this.node().id)
    );
  ...
  _processGrid(propertyValue, propertyName) {
    const splitResult = TextUtils.TextUtils.Utils.splitStringByRegexes(propertyValue, [
      SDK.CSSMetadata.GridAreaRowRegex,
    ]);
  ...
  _innerUpdateTitle() {
    ...
    this.listItemElement
      .createChild('span', 'styles-clipboard-only')
    ...
    if (!this.property.parsedOk) {
      ...
      this.listItemElement.insertBefore(
        StylesSidebarPane.createExclamationMark(this.property),
    ...
    if (this.property.parsedOk && this.section() && this.parent.root) {
      ...
      UI.ARIAUtils.setAccessibleName(
        ...
        `${this.nameElement.textContent} ${this.valueElement.textContent}`
      );
  ...
  _handleContextMenuEvent(context, event) {
    ...
    if (this.property.parsedOk && this.section() && this.parent.root) {
      contextMenu.defaultSection().appendCheckboxItem(
        ls`Toggle property and continue editing`,
        async () => {
  ...
  _editingNameValueKeyPress(context, event) {
    ...
    const isFieldInputTerminated = context.isEditingName
      ? keyChar === ':'
      : keyChar === ';' && shouldCommitValueSemicolon(event.target.textContent, event.target.selectionLeftOffset());
  ...
  async _editingCommitted(userInput, context, moveDirection) {
    ...
    if (
      (moveDirection === 'forward' && (!isEditingName || isPropertySplitPaste)) ||
