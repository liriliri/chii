...
export class ClearStorageView extends UI.ThrottledWidget.ThrottledWidget {
  constructor() {
    ...
    this._reportView.registerRequiredCSS('chi_resources/clearStorageView.css');
    ...
    this._pieChart = new PerfUI.PieChart({
      ...
      size: 110,
      formatter: Number.bytesToString,
      showLegend: true,
    });
  ...
  async doUpdate() {
    ...
    this._quotaRow.textContent = Common.UIString.UIString(
      '%s used out of %s storage quota.\xA0',
      Number.bytesToString(response.usage),
      Number.bytesToString(response.quota)
    );
    if (response.quota < 125829120) {
      // 120 MB
