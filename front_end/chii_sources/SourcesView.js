...
} from './TabbedEditorContainer.js'; // eslint-disable-line no-unused-vars
...
export class SourcesView extends UI.Widget.VBox {
  ...
  constructor() {
    super();
    this.registerRequiredCSS('chii_sources/sourcesView.css');
    ...
    this._editorContainer = new TabbedEditorContainer(
      this,
      Common.Settings.Settings.instance().createLocalSetting('previouslyViewedFiles', []),
    ...
    function handleBeforeUnload(event) {
      ...
      const projects = Workspace.Workspace.WorkspaceImpl.instance().projectsForType(
        Workspace.Workspace.projectTypes.FileSystem
      );
      for (let i = 0; i < projects.length; ++i) {
        unsavedSourceCodes = unsavedSourceCodes.concat(
          projects[i].uiSourceCodes().filter(sourceCode => sourceCode.isDirty())
        );
      }
  ...
  currentSourceFrame() {
    ...
    return /** @type {!UISourceCodeFrame} */ (view);
  }
  ...
  _addUISourceCode(uiSourceCode) {
    ...
    if (
      uiSourceCode.project().type() === Workspace.Workspace.projectTypes.FileSystem &&
  ...
  _updateToolbarChangedListener() {
    ...
    this._toolbarChangedListener = sourceFrame.addEventListener(
      UISourceCodeFrameEvents.ToolbarItemsChanged,
      this._updateScriptViewToolbarItems,
      this
    );
  }
