...
export class JavaScriptBreakpointsSidebarPane extends UI.ThrottledWidget.ThrottledWidget {
  constructor() {
    super(true);
    this.registerRequiredCSS('chii_sources/javaScriptBreakpointsSidebarPane.css');
  ...
  _getBreakpointLocations() {
    const locations = this._breakpointManager
  ...
  async _getContent(locations) {
    ...
    return Promise.all(
      locations.map(async ([breakpointLocation]) => {
  ...
  createElementForItem(item) {
    ...
    if (item.text && lineNumber < item.text.lineCount()) {
      ...
      snippetElement.textContent = lineText
        .substring(item.showColumn ? uiLocation.columnNumber : 0)
  ...
  _breakpointCheckboxClicked(event) {
    ...
    for (const breakpoint of breakpoints) {
      ...
      const item = this._breakpoints.find(breakpointItem =>
        breakpointItem.locations.some(loc => loc.breakpoint === breakpoint)
      );
      if (item) {
  ...
  _revealLocation(element) {
    const uiLocations = this._breakpointLocationsForElement(element).map(
  ...
  _breakpointContextMenu(event) {
    ...
    const removeEntryTitle =
      breakpoints.length > 1
        ? Common.UIString.UIString('Remove all breakpoints in line')
        : Common.UIString.UIString('Remove breakpoint');
    contextMenu
      .defaultSection()
      .appendItem(removeEntryTitle, () => breakpoints.map(breakpoint => breakpoint.remove(false /* keepInStorage */)));
    if (event.target instanceof Element) {
    ...
    const breakpointActiveTitle = breakpointActive
    ...
    contextMenu
      .defaultSection()
      .appendItem(removeOtherTitle, this._removeOtherBreakpoints.bind(this, new Set(breakpoints)));
    contextMenu.show();
  }
...
class BreakpointItem {
  ...
  isSimilar(other) {
    return (
      ...
      this.showColumn === other.showColumn
    );
  }
}
