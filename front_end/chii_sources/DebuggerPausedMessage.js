...
export class DebuggerPausedMessage {
  constructor() {
    ...
    const root = UI.Utils.createShadowRootWithCoreStyles(this._element, 'chii_sources/debuggerPausedMessage.css');
  ...
  static _descriptionWithoutStack(description) {
    ...
    return firstCallFrame
      ? description.substring(0, firstCallFrame.index - 1)
  ...
  async render(details, debuggerWorkspaceBinding, breakpointManager) {
    ...
    const errorLike =
      details.reason === SDK.DebuggerModel.BreakReason.Exception ||
    ...
    if (details.reason === SDK.DebuggerModel.BreakReason.DOM) {
      ...
      messageWrapper = buildWrapper(
        Common.UIString.UIString('Paused on exception'),
        descriptionWithoutStack,
        description
      );
      ...
      const descriptionWithoutStack = DebuggerPausedMessage._descriptionWithoutStack(description);
      messageWrapper = buildWrapper(
        Common.UIString.UIString('Paused on promise rejection'),
        descriptionWithoutStack,
        description
      );
