...
export const scopeIdentifiers = function (scope) {
  ...
  if (
    scope.type() === Protocol.Debugger.ScopeType.Global ||
    !startLocation ||
    !endLocation ||
    !startLocation.script() ||
    !startLocation.script().sourceMapURL ||
...
export const resolveScope = function (scope) {
  ...
  function resolveSourceName(id) {
    ...
    if (
      ...
      !startEntry.sourceLineNumber ||
      !startEntry.sourceColumnNumber ||
      !endEntry.sourceLineNumber ||
...
export const resolveExpression = function (
  ...
  return allVariablesInCallFrame(callFrame).then(reverseMapping =>
    findCompiledName(callFrame.debuggerModel, reverseMapping)
  );
 ...
 * @return {!Promise<string>}
 */
export const resolveExpressionAsync = async function (
  debuggerModel,
  uiSourceCode,
  lineNumber,
  startColumnNumber,
  ...
  const sourceMap = /** @type {!SDK.SourceMap.TextSourceMap} */ (Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().sourceMapForScript(
    script
  ));
  if (!sourceMap) {
    return '';
