...
export class ScriptOriginPlugin extends Plugin {
  ...
  async rightToolbarItems() {
    ...
    if (originURL) {
      const item = UI.UIUtils.formatLocalized('(source mapped from %s)', [
        Components.Linkifier.Linkifier.linkifyURL(originURL),
      ]);
  ...
  static async _script(uiSourceCode) {
    const locations = await Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().uiLocationToRawLocations(
      uiSourceCode,
      0,
      0
    );
