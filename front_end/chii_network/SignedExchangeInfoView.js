...
export class SignedExchangeInfoView extends UI.Widget.VBox {
  ...
  constructor(request) {
    ...
    this.registerRequiredCSS('chii_network/signedExchangeInfoView.css');
    this.element.classList.add('signed-exchange-info-view');

    const root = new UI.TreeOutline.TreeOutlineInShadow();
    root.registerRequiredCSS('chii_network/signedExchangeInfoTree.css');
    ...
    if (signedExchangeInfo.header) {
      ...
      this._responseHeadersItem = headerCategory.createLeaf(
        this._formatHeader(Common.UIString.UIString('Response headers'), '')
      );
      ...
      for (let i = 0; i < header.signatures.length; ++i) {
        ...
        signatureCategory.createLeaf(
          this._formatHeaderForHexData(
            Common.UIString.UIString('Signature'),
            signature.signature,
        ...
        if (signature.certUrl) {
          const certURLElement = this._formatHeader(
            Common.UIString.UIString('Certificate URL'),
            signature.certUrl,
        ...
        signatureCategory.createLeaf(
          this._formatHeader(
            Common.UIString.UIString('Integrity'),
            signature.integrity,
        ...
        if (signature.certSha256) {
          signatureCategory.createLeaf(
            this._formatHeaderForHexData(
              Common.UIString.UIString('Certificate SHA256'),
        ...
        signatureCategory.createLeaf(
          this._formatHeader(
            Common.UIString.UIString('Validity URL'),
    ...
    if (signedExchangeInfo.securityDetails) {
      ...
      securityCategory.createLeaf(
        this._formatHeader(
          Common.UIString.UIString('Valid from'),
          new Date(1000 * securityDetails.validFrom).toUTCString()
        )
      );
      securityCategory.createLeaf(
        this._formatHeader(
          Common.UIString.UIString('Valid until'),
