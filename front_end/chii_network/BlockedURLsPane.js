...
export class BlockedURLsPane extends UI.Widget.VBox {
  constructor() {
    super(true);
    this.registerRequiredCSS('chii_network/blockedURLsPane.css');

    _instance = this;
    this._manager = self.SDK.multitargetNetworkManager;
    this._manager.addEventListener(
      SDK.NetworkManager.MultitargetNetworkManager.Events.BlockedPatternsChanged,
      this._update,
      this
    );
    ...
    this._enabledCheckbox = new UI.Toolbar.ToolbarCheckbox(
      Common.UIString.UIString('Enable request blocking'),
      undefined,
    ...
    const clearButton = new UI.Toolbar.ToolbarButton(
      Common.UIString.UIString('Remove all patterns'),
      'largeicon-clear'
    );
    ...
    this._list.registerRequiredCSS('chii_network/blockedURLsPane.css');
  ...
  _createEditor() {
    ...
    titles.createChild('div').textContent = Common.UIString.UIString(
