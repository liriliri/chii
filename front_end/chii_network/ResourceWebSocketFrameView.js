...
export class ResourceWebSocketFrameView extends UI.Widget.VBox {
  ...
  constructor(request) {
    super();
    this.registerRequiredCSS('chii_network/webSocketFrameView.css');
    ...
    this._frameEmptyWidget = new UI.EmptyWidget.EmptyWidget(
      Common.UIString.UIString('Select message to browse its content.')
    );
  ...
  async _onFrameSelected(event) {
    ...
    this._splitWidget.setSidebarWidget(
      new SourceFrame.ResourceSourceFrame.ResourceSourceFrame(
...
export class ResourceWebSocketFrameNode extends DataGrid.SortableDataGrid.SortableDataGridNode {
  ...
  binaryView() {
    ...
    if (!this._binaryView) {
      this._binaryView = new BinaryResourceView(
        this._dataText,
        /* url */ '',
        Common.ResourceType.resourceTypes.WebSocket
      );
