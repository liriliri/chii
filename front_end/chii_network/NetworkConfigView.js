...
export class NetworkConfigView extends UI.Widget.VBox {
  constructor() {
    super(true);
    this.registerRequiredCSS('chii_network/networkConfigView.css');
  ...
  static createUserAgentSelectAndInput(title) {
    ...
    for (const userAgentDescriptor of userAgentGroups) {
      ...
      for (const userAgentVersion of userAgentDescriptor.values) {
        const userAgentValue = SDK.NetworkManager.MultitargetNetworkManager.patchUserAgentWithChromeVersion(
  ...
  _createNetworkThrottlingSection() {
    ...
    const networkThrottlingSelect = /** @type {!HTMLSelectElement} */ (section.createChild('select', 'chrome-select'));
