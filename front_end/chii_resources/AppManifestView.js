...
export class AppManifestView extends UI.Widget.VBox {
  constructor() {
    super(true);
    this.registerRequiredCSS('chii_resources/appManifestView.css');
  ...
  async _updateManifest(immediately) {
    ...
    this._throttler.schedule(
      () => this._renderManifest(url, data, errors, installabilityErrors, manifestIcons),
      immediately
    );
  }
  ...
  async _renderManifest(url, data, errors, installabilityErrors, manifestIcons) {
    ...
    const setIconMaskedCheckbox = UI.UIUtils.CheckboxLabel.create(
      Common.UIString.UIString('Show only the minimum safe area for maskable icons')
    );
    ...
    this._iconsSection
      .appendRow()
      .appendChild(UI.UIUtils.formatLocalized('Need help? Read our %s.', [documentationLink]));
    ...
    for (const icon of icons) {
      ...
      if (!icon.sizes) {
        ...
        if (image.naturalWidth !== width && image.naturalHeight !== height) {
          imageErrors.push(
            ls`Actual size (${image.naturalWidth}×${image.naturalHeight})px of icon ${iconUrl} does not match specified size (${width}×${height}px)`
          );
        } else if (image.naturalWidth !== width) {
          imageErrors.push(
            ls`Actual width (${image.naturalWidth}px) of icon ${iconUrl} does not match specified width (${width}px)`
          );
        } else if (image.naturalHeight !== height) {
          imageErrors.push(
            ls`Actual height (${image.naturalHeight}px) of icon ${iconUrl} does not match specified height (${height}px)`
          );
        }
      }
      field.appendChild(wrapper);
    ...
    for (const installabilityError of installabilityErrors) {
      ...
      switch (installabilityError.errorId) {
        ...
        case 'manifest-missing-suitable-icon':
          ...
          errorMessage = ls`Manifest does not contain a suitable icon - PNG, SVG or WebP format of at least ${installabilityError.errorArguments[0].value}px is required, the sizes attribute must be set, and the purpose attribute, if set, must include "any" or "maskable".`;
          break;
        ...
        case 'no-acceptable-icon':
          ...
          errorMessage = ls`No supplied icon is at least ${installabilityError.errorArguments[0].value}px square in PNG, SVG or WebP format`;
          break;
