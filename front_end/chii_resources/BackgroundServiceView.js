...
export class BackgroundServiceView extends UI.Widget.VBox {
  ...
  constructor(serviceName, model) {
    super(true);
    this.registerRequiredCSS('chii_resources/backgroundServiceView.css');
    ...
    this._securityOriginManager.addEventListener(SDK.SecurityOriginManager.Events.MainSecurityOriginChanged, () =>
      this._onOriginChanged()
    );

    /** @const {!UI.Action.Action} */
    this._recordAction = /** @type {!UI.Action.Action} */ (self.UI.actionRegistry.action(
  ...
  async _setupToolbar() {
    ...
    this._originCheckbox = new UI.Toolbar.ToolbarCheckbox(
      ls`Show events from other domains`,
      ls`Show events from other domains`,
      () => this._refreshView()
    );
  ...
  _createDataGrid() {
    ...
    dataGrid.addEventListener(DataGrid.DataGrid.Events.SelectedNode, event =>
      this._showPreview(/** @type {!EventDataNode} */ (event.data))
    );
  ...
  _showPreview(dataNode) {
    ...
    if (this._dataGrid.rootNode().children.length) {
      ...
      centered.createChild(
        'p'
      ).textContent = ls`DevTools will record all ${featureName} activity for up to 3 days, even when closed.`;
    } else {
      ...
      recordKey.textContent = self.UI.shortcutRegistry.shortcutDescriptorsForAction(
        'background-service.toggle-recording'
      )[0].name;
      ...
      centered
        .createChild('p')
        .appendChild(
          UI.UIUtils.formatLocalized('Click the record button %s or hit %s to start recording.', [
            inlineButton,
            recordKey,
          ])
        );
  ...
  createPreview() {
    ...
    for (const entry of this._eventMetadata) {
      ...
      if (entry.value) {
        ...
        div.createChild(
          'div',
          'background-service-metadata-value background-service-empty-value'
        ).textContent = ls`empty`;
      }
