...
export class ServiceWorkersView extends UI.Widget.VBox {
  constructor() {
    super(true);
    this.registerRequiredCSS('chii_resources/serviceWorkersView.css');
    ...
    const updateOnReloadSetting = Common.Settings.Settings.instance().createSetting(
    ...
    const forceUpdate = new UI.Toolbar.ToolbarSettingCheckbox(
      updateOnReloadSetting,
      ls`On page reload, force the service worker to update, and activate it`
    );
    ...
    const fallbackToNetwork = new UI.Toolbar.ToolbarSettingCheckbox(
      ...
      ls`Bypass the service worker and load resources from the network`
    );
  ...
  modelAdded(serviceWorkerManager) {
    ...
    this._eventListeners.set(serviceWorkerManager, [
      ...
      this._manager.addEventListener(
        SDK.ServiceWorkerManager.Events.RegistrationDeleted,
        this._registrationDeleted,
        this
      ),
  ...
  _updateSectionVisibility() {
    ...
    for (const section of this._sections.values()) {
      if (
        section._section.parentWidget() === this._currentWorkersView ||
  ...
  _updateRegistration(registration, skipUpdate) {
    ...
    if (!section) {
      ...
      section = new Section(
        /** @type {!SDK.ServiceWorkerManager.ServiceWorkerManager} */ (this._manager),
        uiSection,
        registration
      );
...
export class Section {
  ...
  constructor(manager, section, registration) {
    ...
    this._pushNotificationDataSetting = Common.Settings.Settings.instance().createLocalSetting(
      'pushData',
      Common.UIString.UIString('Test push message from DevTools.')
    );
    this._syncTagNameSetting = Common.Settings.Settings.instance().createLocalSetting(
      'syncTagName',
      'test-tag-from-devtools'
    );
    this._periodicSyncTagNameSetting = Common.Settings.Settings.instance().createLocalSetting(
      'periodicSyncTagName',
      'test-tag-from-devtools'
    );
    ...
    this._updateButton = new UI.Toolbar.ToolbarButton(
      Common.UIString.UIString('Update'),
      undefined,
      Common.UIString.UIString('Update')
    );
    ...
    this._deleteButton = new UI.Toolbar.ToolbarButton(
      Common.UIString.UIString('Unregister service worker'),
      undefined,
      Common.UIString.UIString('Unregister')
    );
    ...
    this._createSyncNotificationField(
      Common.UIString.UIString('Push'),
      this._pushNotificationDataSetting.get(),
      Common.UIString.UIString('Push data'),
      this._push.bind(this)
    );
    this._createSyncNotificationField(
      Common.UIString.UIString('Sync'),
      this._syncTagNameSetting.get(),
      Common.UIString.UIString('Sync tag'),
      this._sync.bind(this)
    );
    this._createSyncNotificationField(
      ls`Periodic Sync`,
      this._periodicSyncTagNameSetting.get(),
      ls`Periodic Sync tag`,
  ...
  _createSyncNotificationField(label, initialValue, placeholder, callback) {
    const form = this._wrapWidget(this._section.appendField(label)).createChild(
      'form',
    ...
    for (const client of version.controlledClients) {
      ...
      if (this._clientInfoCache.has(client)) {
        this._updateClientInfo(
          clientLabelText,
          /** @type {!Protocol.Target.TargetInfo} */ (this._clientInfoCache.get(client))
        );
      }
  ...
  _updateSourceField(version) {
    ...
    this._sourceField.createChild('div', 'report-field-value-subtitle').textContent = Common.UIString.UIString(
      'Received %s',
      new Date(version.scriptResponseTime * 1000).toLocaleString()
    );
  }
  ...
  _update() {
    ...
    if (active) {
      ...
      const activeEntry = this._addVersion(
        ...
        ls`#${active.id} activated and is ${localizedRunningStatus}`
      );
      ...
      if (active.isRunning() || active.isStarting()) {
        ...
        if (!this._targetForVersionId(active.id)) {
          this._createLink(
            activeEntry,
            Common.UIString.UIString('inspect'),
      ...
      this._addVersion(
        ...
        Common.UIString.UIString('#%s is redundant', redundant.id)
      );
    ...
    if (waiting) {
      ...
      waitingEntry.createChild('div', 'service-worker-subtitle').textContent = Common.UIString.UIString(
        'Received %s',
    ...
    if (installing) {
      ...
      installingEntry.createChild('div', 'service-worker-subtitle').textContent = Common.UIString.UIString(
        'Received %s',
  ...
  _updateClientInfo(element, targetInfo) {
    ...
    this._createLink(
      element,
      ls`focus`,
      this._activateTarget.bind(this, targetInfo.targetId),
